...
export interface PanelDef {
  ...
  animation?: 'async-generator' | 'transcript' | 'microcompact' | 'snip' | 'context-collapse' | 'autocompact' | 'flow-overview' | 'per-tool' | 'streaming-executor' | 'concurrency-partition' | 'tools' | 'understand' | 'extraction' | 'prefetch' | 'cache' | 'api-stream' | 'stop-hooks' | 'context-inheritance' | 'agent-tool-flow' | 'system-prompt' | 'session-memory'
  layout?: 'split'
}
...
export interface SceneDef {
  ...
  sideSteps?: { text: string; y: number; fromBoxId?: string }[]  // step boxes pinned at y% alongside the diagram
}
